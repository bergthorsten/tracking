...
import { MONTH_TOTALS } from "@/data/mock"
import { formatDuration, monthLabel } from "@/lib/time"

const MONTH_OPTIONS = Object.keys(MONTH_TOTALS).map((key) => {
  const [year, month] = key.split("-").map(Number)
  return { value: key, label: monthLabel(year, month) }
})

export function MockMonthSummary() {
